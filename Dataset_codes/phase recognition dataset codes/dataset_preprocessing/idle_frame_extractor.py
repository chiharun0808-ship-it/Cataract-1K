...
case_list = [d for d in os.listdir(path) if d.startswith("case") and os.path.isdir(os.path.join(path, d))]
case_list.sort()
...
def convert_frames_to_video(vid,frame_1_secs_str, duration_secs_str, idle_folder, name):
    ...
    # ダブルクォーテーションで囲んで安全にする
    output_path = os.path.join(idle_folder, name)
    cmd = f'ffmpeg -i "{vid}" -ss {frame_1_secs_str} -t {duration_secs_str} -c:v copy -c:a copy "{output_path}"'
...
for i in range(0,len(case_list)):
    ...
    video_annot_csv = case_folder + '/' + case_list[i] + '_annotations_phases.csv'
    #video = video_path + case_list[i] + '/' + case_list[i] + '.mp4'
    # フォルダ名を挟まず、直接ファイル名を結合するように変更
    ...
    for j in range(len(annot)-1):
        ...
        if (frame_stop-frame_start)/fps >3:
            ...
            # コロンをハイフンに置き換える処理を追加
            time_start_str = frame_1_secs_str.replace(':', '-')
            time_end_str = str(secs_to_timedelta(frame_2_secs)).replace(':', '-')
            name = case_list[i] + '_idle_' + time_start_str + '-' + time_end_str + '.mp4'
